...
import Function.BP_QMS_Decoder as BP_QMS_Decoder

class BP_Training:
    def __init__(self, train_config_in, code_in):
        #config
    ...
    def build_network(self, bp_decoder):
        #x_in
        ...
        self.beta = tf.Variable(1*tf.ones([1]), dtype=tf.float32, name=self.beta_name)
    ...
    def cal_training_loss(self, y_out, y_label):   
        ...
        o_out = tf.div(1-self.softsign(tf.slice(y_out, [0,0],[self.train_config.label_length, self.train_config.training_minibatch_size])), 2)
        training_loss = tf.reduce_mean(tf.square(o_out-tf.transpose(y_label)))
        return training_loss
    
    def train_network(self, BP_iter_nums, SNR_set):
        start = datetime.datetime.now()
        bp_decoder = BP_QMS_Decoder.BP_NetDecoder(self.code.H_matrix, self.train_config.training_minibatch_size)
        ...
        sess = tf.Session()     
        for SNR in SNR_set:       
            training_feature_file = format('%s_%.1f.dat' % (self.train_config.training_feature_file, SNR))
            training_label_file = format('%s_%.1f.dat' % (self.train_config.training_label_file, SNR))
            test_feature_file = format('%s_%.1f.dat' % (self.train_config.test_feature_file, SNR))
            test_label_file = format('%s_%.1f.dat' % (self.train_config.test_label_file, SNR))
            ...
            for iteration in range(0, BP_iter_nums[0]):
                ...
                para_file = format('%sPARA(%d_%d)_SNR%.1f_Iter%d.txt' % (self.train_config.para_folder, self.train_config.feature_length, self.train_config.label_length, SNR, iteration+1))
